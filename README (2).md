...
- [Notebooks](#notebooks)
  ...
  - [Notebook 3 - Unified Five-Class Model](#notebook-3--unified-five-class-classifier-real--4-manipulations)
...
| `deepfake_multiclass_v3_PC.ipynb` | Unified detection + classification | MobileNetV2 (+ MixUp + TTA) | Real / Deepfakes / Face2Face / FaceSwap / NeuralTextures |
...
├── deepfake_multiclass_v3_PC.ipynb ← Notebook 3: Unified 5-Class (Real + 4 manipulations)
│
...
### Notebook 3  Unified Five-Class Classifier (Real + 4 Manipulations)

**Goal:** Do detection **and** attribution in a single model classify any frame directly into one of five classes: **Real, Deepfakes, Face2Face, FaceSwap, NeuralTextures**. Unlike Notebook 2, this includes both real frames and the previously excluded NeuralTextures family.

> This is the hardest setting in the project: the model must simultaneously reject reality and separate four visually similar forgeries.

#### Pipeline
```
All Frames (5 classes) → Index Building → 3-Stage Training → TTA Inference → Evaluation
```

#### Key Components

**Model  MobileNetV2 (5-class head):**
- ImageNet-pretrained MobileNetV2 backbone with a custom 5-way classifier head.
- Trained with **MixUp** and **label smoothing** to smooth the tight boundaries between manipulation families.

**Three-Stage Training:**
- **Stage 1:** Classifier head only (backbone frozen).
- **Stage 2:** Partial unfreezing of the upper backbone blocks.
- **Stage 3:** Full fine-tuning at a low learning rate, with early stopping.

**Test-Time Augmentation (TTA):**
At inference, predictions are averaged over **7 augmented views** of each frame (flips/crops). TTA alone raises accuracy from **73.70%** to **78.95%** and is the default reported configuration.

**Top-2 Accuracy:**
In addition to standard metrics, Top-2 accuracy (the correct class is among the model's two highest-scoring guesses) reaches **93.33%** useful for a triage tool where a short shortlist is acceptable.
...
### Unified Five-Class Model (MobileNetV2 + TTA)

Single model over **Real, Deepfakes, Face2Face, FaceSwap, NeuralTextures**.

| Metric | Standard | With TTA |
|---|---|---|
| Accuracy | 73.70% | **78.95%** |
| Macro-F1 | 0.736 | **0.792** |
| OvR Macro-AUC | | **0.965** |
| Top-2 Accuracy | | **93.33%** |

**Per-class (frame-level, with TTA):**

| Class | Precision | Recall | F1 |
|---|---|---|---|
| Real | 0.68 | 0.90 | 0.77 |
| Deepfakes | 0.85 | 0.65 | 0.74 |
| Face2Face | 0.98 | 0.72 | 0.83 |
| FaceSwap | 0.90 | 0.90 | 0.90 |
| NeuralTextures | 0.77 | 0.67 | 0.72 |

> FaceSwap is the cleanest class; NeuralTextures and Deepfakes are the hardest the price of covering every manipulation plus real in one model. Test-time augmentation adds about +5% accuracy.
## Visualizations
...
### Step 4b  Run Notebook 3 (Unified Five-Class)

Open `deepfake_multiclass_v3_PC.ipynb`. It reuses the same extracted frames (real + all four manipulations). Run all cells in order; the final cells run the three training stages, evaluate with test-time augmentation, and save the five-class visualizations.
...
- **NeuralTextures in the 3-class model:** The dedicated manipulation classifier (Notebook 2) excludes NeuralTextures to keep the 3-class boundary tractable. The unified five-class model (Notebook 3) **does** include it, at the cost of lower overall accuracy NeuralTextures remains the hardest class to separate.
